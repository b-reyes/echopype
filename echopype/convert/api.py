...
def open_raw(
    ...
    if sonar_model in ["EK60", "ES70", "EK80", "ES80", "EA640"]:
        ...
        # Determines if writing to zarr is necessary and writes to zarr
        p2z_flag = (
            offload_to_zarr is True
            or (offload_to_zarr == "auto" and p2z.write_to_zarr(mem_mult=0.4))
        )
        ...
        if p2z_flag:
            ...
            p2z = Parsed2Zarr(parser)
            parser.rectangularize_data()  # TX PART SEPARATED OUT FROM THIS FUNCTION FOR CLARITY

        # Rectangularize the transmit data
        parser.rectangularize_transmit_ping_data()  # ARGUMENT data_type REMOVED
