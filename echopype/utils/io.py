...
def save_file(ds, path, mode, engine, group=None, compression_settings=None, compute=True):
    ...
    if engine == "netcdf4":
        return ds.to_netcdf(path=path, mode=mode, group=group, encoding=encoding, compute=compute)
    elif engine == "zarr":
        return ds.to_zarr(store=path, mode=mode, group=group, encoding=encoding, compute=compute)
    else:
